                     ...
                     char **ofile)
{
    ...
    if ( !(str = (strfile_t *) malloc(sizeof(strfile_t))) )
		return 0;
	str->raw.obs.data  = NULL; str->raw.obs.n = 0; //FF
	str->raw.obuf.data = NULL; str->raw.obuf.n= 0;
	str->raw.nav.eph   = NULL; str->raw.nav.n = 0;
	str->raw.nav.alm   = NULL; str->raw.nav.na = 0;
	str->raw.nav.geph  = NULL; str->raw.nav.ng = 0;
	str->raw.nav.seph  = NULL; str->raw.nav.ns = 0;
    ...
    for (i = 0; i < nf && !abort; i++) {
        ...
        for (j = 0; (type = input_strfile(str)) >= -1; j++) {
            ...
            if ( opt->te.time&&timediff(te, opt->te) > 10.0 )
				break;
        }
        /* close stream file */
        close_strfile(str);
	...
	for (i = 0; i < NOUTFILE; i++) { // FF 
		strcpy(ofile[i], paths[i]);
	}
    return abort ? -1 : 1;
}
/* rinex converter -------------------------------------------------------------
...
int raw2rnx1(const char* file, const char* ext)
{
	int format = -1;
	rnxopt_t opt;
	char* ofile[6];
	...
	memset(&opt, 0, sizeof(rnxopt_t));
	
	if ( !stricmp(ext, "ubx") ) 
		format = STRFMT_UBX;
	else if ( !stricmp(ext, "jvs") ) 
		format = STRFMT_JAVAD;
	if ( format < 0 )
		return 0;

	return convrnx(format, &opt, file, &ofile[0]);
}

int raw2rnx(const char* nfile, const char* ext, char** of)
{
    rnxopt_t rnxopt = {0};
    //std::string InFile_Text = nfile;
 //   std::string OutFile1_Text ="D:\\temp\\ublox\\%r%n0.%yO";
	//std::string OutFile2_Text = "D:\\temp\ublox\\%r%n0.%yN";
 //   std::string OutFile3_Text, OutFile4_Text;
 //   std::string OutFile5_Text, OutFile6_Text;
 //   std::string OutFile7_Text;
    int i, format, sat;
	...
	//strcpy(ofile[0],"D:\\temp\\ublox\\%r%n0.%yO");
	//strcpy(ofile[1], "D:\\temp\\ublox\\%r%n0.%yN");
  //  if (OutFile3->Enabled&&OutFileEna3->Checked) 
		//strcpy(ofile[2],OutFile3_Text.c_str());
  //  if (OutFile4->Enabled&&OutFileEna4->Checked)
		//strcpy(ofile[3],OutFile4_Text.c_str());
  //  if (OutFile5->Enabled&&OutFileEna5->Checked) 
		//strcpy(ofile[4],OutFile5_Text.c_str());
  //  if (OutFile6->Enabled&&OutFileEna6->Checked) 
		//strcpy(ofile[5],OutFile6_Text.c_str());
  //  if (OutFile7->Enabled&&OutFileEna7->Checked) 
		//strcpy(ofile[6],OutFile7_Text.c_str());
 ...
 //   }
    ...
    sprintf(rnxopt.prog, "%s %s", "Geoin", "2.0");
  ...
  //  for (i = 0; i < 2; i++)
		//strncpy(rnxopt.name[i], Name[i].c_str(), 31);
  //  for (i = 0; i < 3; i++)
		//strncpy(rnxopt.rec[i], Rec[i].c_str(), 31);
  //  for (i = 0; i < 3; i++) 
		//strncpy(rnxopt.ant[i], Ant[i].c_str(), 31);
    //if ( AutoPos ) {
        for (i = 0; i < 3; i++) 
			rnxopt.apppos[i] = 0;
    //}
    ...
    if ( *rnxopt.rcvopt )
		sprintf(p,", option: %s", rnxopt.rcvopt);
  //  for (i = 0; i < 2; i++) 
		//strncpy(rnxopt.comment[i+2], Comment[i].c_str(), 63);
  //  for (i = 0; i < 6;i++) 
		//strcpy(rnxopt.mask[i], CodeMask[i].c_str());
    ...
    // convert to rinex
	ret = convrnx(format, &rnxopt, file, ofile);
	if ( ret == 1 ) {
		for (i = 0; i < 7; i++) {
			if ( strlen(ofile[i]) )
				strcpy(of[i], ofile[i]);
