...
TOOLS_EXPORTS int crx2rnx(const char* crx)
{
    ...
    if ( fileopen(argc, argv) < 0 ) {
		exit_status = EXIT_FAILURE;
		goto FINE;
	}

    for(i  = 0; i < UCHAR_MAX; i++)
		ntype_gnss[i] = -1;  /** -1 unless GNSS type is not defined **/
...
SKIP:
        ...
        if ( dline[0] == ep_top_from ) {
            ...
            if ( *p_event!='0' && *p_event!='1' ) {
				int kkk = put_event_data(dline, p_event);
				if ( kkk == -1 ) {
					exit_status = EXIT_FAILURE;
					goto FINE; // errore
				}
				if ( kkk == 2 )
					goto FINE; // fine corretta
                //if ( put_event_data(dline, p_event) != 0 )
				if ( kkk != 0 )
					if ( skip_to_next(dline) == -1 )
						goto FINE;
                goto SKIP;
            }
        ...
        if ( set_sat_table(p_satlst, sat_lst_old, nsat1, sattbl) ) /****  set satellite table  ****/
			goto FINE;
		kkk = read_chk_line(dline);
        //if ( read_chk_line(dline) != 0 ) {
		if ( kkk < 0 ) {
			exit_status = EXIT_FAILURE;
			goto FINE;
		}
        ...
        if ( read_clock(dline, clk1.u, clk1.l) < 0 ) {
			exit_status = EXIT_FAILURE;
			goto FINE;
		}
        for (i = 0, i0 = sattbl; i < nsat; i++, i0++) {
            ntype = ntype_record[i];
			kkk = getdiff(y1[i], y0[*i0], *i0, dflag[i]);
			if ( kkk < 0 )
				goto FINE;
        ...
        if ( rinex_version == 2 ) {
            ...
            for(p = &line[68], n=nsat-12; n > 0; n -= 12, p += 36) 
				p_buff += sprintf(p_buff,"%32.s%.36s\n"," ",p);
        } else {
            if( clk_order >= 0){
...
int fileopen(int argc, char *argv[])
{
    ...
    if ( help == 1 || nfile > 1  || nfile < 0 )
		return  error_exit(2, progname);
    if ( nfile == 0 ) 
		return 0;       /*** stdin & stdout will be used ***/

    /***********************/
    /*** open input file ***/
    /***********************/
    p = strrchr(infile,'.');
    if ( p == NULL || (*(p+3) != 'D' && *(p+3) != 'd') || *(p+4) != '\0' ) 
		return error_exit(3,p);
    if ( (ifp = fopen(infile,"r")) == NULL) 
		return error_exit(4, infile);
    ...
    //        fprintf(fperr,"The file %s already exists. Overwrite?(n)",outfile);
    //        if ( getchar() != 'y') 
				//exit(EXIT_SUCCESS);
    //    }
...
{
    char line[MAXCLM], *p;
	int kkk = read_chk_line(line);
    if ( kkk != 0 ) 
		return error_exit(5, "1.0-2.0");
	...
	kkk =  read_chk_line(line);
    //if ( read_chk_line(line) == 1 ) 
    if ( kkk != 0 ) 
		return error_exit(8,line);
    ...
    if( strncmp(&line[60],"RINEX VERSION / TYPE",20) != 0 ||
       (line[5]!='2' && line[5]!='3') ) 
	   return error_exit(15,"2.x or 3.x");
    rinex_version = atoi(line);

    do {
        if ( read_chk_line(line) != 0 )
			return -1;
        CHOP_BLANK(line, p);
        printf("%s\n", line);
        if ( strncmp(&line[60],"# / TYPES OF OBSERV",19) == 0 && line[5] != ' ' ) {
             ...
             if ( line[0] != ' ' ) 
				 ntype_gnss[line[0]] = atoi(&line[3]);
             if ( ntype_gnss[line[0]] > MAXTYPE ) 
				 return error_exit(15,line);
        }
...
int read_clock(char *dline, long *yu, long *yl)
{
    ...
    if ( *p == '\0') {
        ...
        if ( *(p+1) == '&' ) {        /**** for the case of arc initialization ****/
            ...
            if ( clk_arc_order > MAX_DIFF_ORDER )
				return error_exit(7, dline);
            clk_order = -1;
            p += 2;
        }
        p1 = p; 
...
/***********************************************************************/
    ...
    do {
        ...
        if ( strlen(dline) > 29 ) {
            ...
            for (i = 0; i < n; i++) {
                ...
                if (strncmp(&dline[60], "# / TYPES OF OBSERV", 19) == 0 && dline[5] != ' ' ) {
                     ...
                     if ( dline[0] != ' ' ) 
						 ntype_gnss[dline[0]] = atoi(&dline[3]);
                     if ( ntype_gnss[dline[0]] > MAXTYPE ) 
						 return error_exit(15,dline);
                }
            }
        }
        ...
        do {
            ...
            if( fgets(dline,MAXCLM, stdin) == NULL ) 
				return 0;
				//exit(exit_status);  /*** eof: exit program successfully ***/
        } while ( crinex_version == 3 && dline[0] == '&' );
        CHOP_LF(dline,p);

        if ( dline[0] != ep_top_from || strlen(dline)<29   || ! isdigit(*p_event) ) {
            if ( !skip )
				return error_exit(9, dline);
            fprintf(fperr,"WARNING :  The epoch should be initialized, but not.\n");
            return 1;
        }
...
int skip_to_next(char *dline)
{
    ...
    fprintf(fperr,"    line %ld : skip until an initialized epoch is found.",nl_count);
    if ( rinex_version == 2 ) {
    ...
    do {
        nl_count++;
        if ( fgets(dline, MAXCLM, stdin) == NULL ) {
            fprintf(fperr,"  .....next epoch not found before EOF.\n");
			return -1;
            //exit(exit_status);
        }
    ...
    CHOP_LF(dline,p);
    fprintf(fperr,"  .....next epoch found at line %ld.\n",nl_count);
	return 0;
}
/*---------------------------------------------------------------------*/
int set_sat_table(char *p_new, char *p_old, int nsat1, int *sattbl)
        ...
        if ( *s1 == '\0' ) {
            ...
            if ( *(s1+1) == '&' ) {     /**** arc initialization ****/
                ...
                if ( y->arc_order > MAX_DIFF_ORDER ) 
					return error_exit(7, line);
            } else if (i0 < 0){
                if ( ! skip )
					return error_exit(11,line);
                fprintf(fperr, "WARNING : New satellite, but data arc is not initialized.\n");
                return 1;
            } else if ( y0->arc_order < 0 ) {
                if ( ! skip ) 
					return error_exit(12,line);
                fprintf(fperr, "WARNING : New data sequence but without initialization.\n");
                return 1;
            } else{
                y->order = y0->order;
...
{
    if ( error_no == 1 ) {
        fprintf(fperr,"Usage: %s input file [-o output file] [-f] [-s] [-h]\n",string);
        fprintf(fperr,"    output file name can be omitted if input file name is *.[yy]d\n");
    } else if( error_no == 2 ) {
        fprintf(fperr,"Usage: %s [file] [-] [-f] [-s] [-h]\n",string);
        fprintf(fperr,"    stdin and stdout are used if input file name is not given.\n");
    }
    if ( error_no == 1 || error_no == 2 ) {
        fprintf(fperr,"    -  : output to stdout\n");
        fprintf(fperr,"    -f : force overwrite of output file\n");
        fprintf(fperr,"    -s : skip strange epochs (default:stop with error)\n");
        fprintf(fperr,"           This option may be used for salvaging usable data when middle of\n");
        fprintf(fperr,"           the Compact RINEX file is missing. The data after the missing part,\n");
        fprintf(fperr,"           are, however, useless until the compression operation of all data\n");
        fprintf(fperr,"           are initialized at some epoch. Combination with use of -e option\n");
        fprintf(fperr,"           of RNX2CRX (ver.4.0 or after) may be effective.\n");
        fprintf(fperr,"           Caution : It is assumed that no change in # of data types\n");
        fprintf(fperr,"                     happens in the lost part of the data.\n");
        fprintf(fperr,"    -h : display help message\n\n");
        fprintf(fperr,"    exit code = %d (success)\n",EXIT_SUCCESS);
        fprintf(fperr,"              = %d (error)\n",  EXIT_FAILURE);
        fprintf(fperr,"              = %d (warning)\n",EXIT_WARNING);
        fprintf(fperr,"    [version : %s]\n",VERSION);
        return -1;
    }
    if( error_no == 3   ){
        fprintf(fperr,"ERROR : invalid file name  %s\n",string);
        fprintf(fperr,"The extention of the input file name should be [.xxd].\n");
        fprintf(fperr,"If the file name doesn't obey this naming convention, use this program as a filter. \n");
        fprintf(fperr,"    for example)  cat file.in | %s - > file.out\n",PROGNAME);
         return -1;
    }
    if ( error_no == 4 ){
        fprintf(fperr,"ERROR : can't open %s\n",string);
         return -1;
    }
    if ( error_no == 5 ){
        fprintf(fperr,"ERROR : The file format is not Compact RINEX or the version of\n");
        fprintf(fperr,"        the format is not valid. This software can deal with\n");
        fprintf(fperr,"        only Compact RINEX format ver.%s.\n",string);
         return -1;
    }
    if ( error_no == 6 ) {
        fprintf(fperr,"ERROR at line %ld : exceed maximum number of satellites(%d)\n",nl_count,MAXSAT);
        fprintf(fperr,"      start>%s<end\n",string);
         return -1;
    }
    if(error_no == 7 ){
        fprintf(fperr,"ERROR at line %ld : exceed maximum order of difference (%d)\n",nl_count,MAX_DIFF_ORDER);
        fprintf(fperr,"      start>%s<end\n",string);
         return -1;
    }
    if ( error_no == 8 ){
        fprintf(fperr,"ERROR : The file seems to be truncated in the middle.\n");
        fprintf(fperr,"        The conversion is interrupted after reading the line %ld :\n",nl_count);
        fprintf(fperr,"      start>%s<end\n",string);
        return -1;
    }
    if ( error_no == 9 ) {
        fprintf(fperr,"ERROR at line %ld : The arc should be initialized, but not.\n",nl_count);
        fprintf(fperr,"      start>%s<end\n",string);
         return -1;
    }
    if ( error_no == 11){
        fprintf(fperr,"ERROR at line %ld : New satellite, but data arc is not initialized.\n",nl_count);
        fprintf(fperr,"      start>%s<end\n",string);
        return -1;
    }
    if (error_no == 12) {
        fprintf(fperr,"ERROR at line %ld : The data field in previous epoch is blank, but the arc is not initialized.\n",nl_count);
        fprintf(fperr,"      start>%s<end\n",string);
         return -1;
    }
    if (error_no == 13) {
        fprintf(fperr,"ERROR at line %ld : null character is found in the line or the line is too long (>%d) at line.\n",nl_count,MAXCLM);
        fprintf(fperr,"      start>%s<end\n",string);
         return -1;
    }
    if (error_no == 15 ) {
        fprintf(fperr,"ERROR : The format version of the original RINEX file is not valid.\n");
        fprintf(fperr,"         This software can deal with only (compressed) RINEX format ver.%s.\n",string);
         return -1;
    }
    if(error_no == 20 ){
        fprintf(fperr,"ERROR at line %ld. : A GNSS type not defined in the header is found.\n",nl_count);
        fprintf(fperr,"     start>%s<end\n",string);
        return -1;
    }
	return 0;
