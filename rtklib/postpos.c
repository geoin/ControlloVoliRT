                      ...
                      const solopt_t *sopt)
{
    ...
    if ( PMODE_DGPS <= popt->mode && popt->mode <= PMODE_FIXED && popt->mode != PMODE_MOVEB ) {
		double rpos[3]; // coordinate base station
		if ( fp )
		...
		ecef2pos(popt->rb, rpos);	
		myCoordPlot(rpos, NULL);
    }
...
static int inputobs(obsd_t *obs, int solq, const prcopt_t *popt)
{
    ...
    if ( 0 <= iobsu&&iobsu<obss.n ) {
		if ( settime((time = obss.data[iobsu].time)) ) {
			aborts = 1;
			showmsg("aborted");
			return -1;		}
    ...
    if ( !revs ) { /* input forward data */
        ...
        if (fp_rtcm) {
            ...
            while (input_rtcm3f(&rtcm,fp_rtcm)>=0) {
                if (timediff(rtcm.time,obs[0].time)>=5.0)
					break;
            }
            for (i = 0; i < MAXSAT ;i++) 
                    ...
                    int mode)
{
	...
	int checklim = !(popt->xlim[0] == 0. && popt->xlim[1] == 0.);
    ...
    while ( (nobs=inputobs(obs,rtk.sol.stat,popt)) >= 0 ) {
		...
		// FF
		ecef2pos(rtk.sol.rr, rpos);
		if ( checklim && (
			rpos[0] < popt->ylim[0] || rpos[0] > popt->ylim[1] || rpos[1] < popt->xlim[0] || rpos[1] > popt->xlim[1]) )
			continue;
                        ...
                        nav_t *nav, sbs_t *sbs, lex_t *lex)
{
    ...
    for (i=0;i<n;i++) {
        ...
        if (ext[strlen(ext)-1]!='i'&&ext[strlen(ext)-1]!='I') 
			continue;
    ...
    for (i=0;i<n;i++) {
        ...
        if ( !(ext=strrchr(infile[i],'.')) )
			continue;
        if ( strcmp(ext,".rtcm3")&&strcmp(ext,".RTCM3") )
			continue;
                  ...
                  const prcopt_t *opt)
{
    ...
    for (iobs = 0;(m=nextobsf(obs, &iobs, rcv)) > 0; iobs += m) {
        ...
        if ( !pntpos(data,j,nav,opt,&sol,NULL,NULL,msg, dop)) 
			continue;
