...
int GNprintf(char* fmt, ...)
{
	...
	strcpy(format, fmt);
	if ( (retval = vsprintf(string, (char*) format, arglist)) > 0 )
		(*fGNprintf)(string);
	return 0;
}

#define TEPS 1.e-6 //1.e-10
//#ifdef SINGLE
//#define REAL float
//#else /* not SINGLE */
#define REAL double
...
void Terminate(short k);
static char	TrErrMes[256];
...
#ifdef CDT_ONLY
#ifdef REDUCED
  GNprintf("triangle [-pAcjevngBPNEIOXzo_lQVh] input_file\n");
#else /* not REDUCED */
  GNprintf("triangle [-pAcjevngBPNEIOXzo_iFlCQVh] input_file\n");
#endif /* not REDUCED */
#else /* not CDT_ONLY */
#ifdef REDUCED
  GNprintf("triangle [-prq__a__uAcDjevngBPNEIOXzo_YS__lQVh] input_file\n");
#else /* not REDUCED */
  GNprintf("triangle [-prq__a__uAcDjevngBPNEIOXzo_YS__iFlsCQVh] input_file\n");
#endif /* not REDUCED */
#endif /* not CDT_ONLY */

  GNprintf("    -p  Triangulates a Planar Straight Line Graph (.poly file).\n");
#ifndef CDT_ONLY
  GNprintf("    -r  Refines a previously generated mesh.\n");
  GNprintf(
    "    -q  Quality mesh generation.  A minimum angle may be specified.\n");
  GNprintf("    -a  Applies a maximum triangle area constraint.\n");
  GNprintf("    -u  Applies a user-defined triangle constraint.\n");
#endif /* not CDT_ONLY */
  GNprintf(
    "    -A  Applies attributes to identify triangles in certain regions.\n");
  GNprintf("    -c  Encloses the convex hull with segments.\n");
#ifndef CDT_ONLY
  GNprintf("    -D  Conforming Delaunay:  all triangles are truly Delaunay.\n");
#endif /* not CDT_ONLY */
/*
  GNprintf("    -w  Weighted Delaunay triangulation.\n");
  GNprintf("    -W  Regular triangulation (lower hull of a height field).\n");
*/
  GNprintf("    -j  Jettison unused vertices from output .node file.\n");
  GNprintf("    -e  Generates an edge list.\n");
  GNprintf("    -v  Generates a Voronoi diagram.\n");
  GNprintf("    -n  Generates a list of triangle neighbors.\n");
  GNprintf("    -g  Generates an .off file for Geomview.\n");
  GNprintf("    -B  Suppresses output of boundary information.\n");
  GNprintf("    -P  Suppresses output of .poly file.\n");
  GNprintf("    -N  Suppresses output of .node file.\n");
  GNprintf("    -E  Suppresses output of .ele file.\n");
  GNprintf("    -I  Suppresses mesh iteration numbers.\n");
  GNprintf("    -O  Ignores holes in .poly file.\n");
  GNprintf("    -X  Suppresses use of exact arithmetic.\n");
  GNprintf("    -z  Numbers all items starting from zero (rather than one).\n");
  GNprintf("    -o2 Generates second-order subparametric elements.\n");
#ifndef CDT_ONLY
  GNprintf("    -Y  Suppresses boundary segment splitting.\n");
  GNprintf("    -S  Specifies maximum number of added Steiner points.\n");
#endif /* not CDT_ONLY */
#ifndef REDUCED
  GNprintf("    -i  Uses incremental method, rather than divide-and-conquer.\n");
  GNprintf("    -F  Uses Fortune's sweepline algorithm, rather than d-and-c.\n");
#endif /* not REDUCED */
  GNprintf("    -l  Uses vertical cuts only, rather than alternating cuts.\n");
#ifndef REDUCED
#ifndef CDT_ONLY
  GNprintf(
    "    -s  Force segments into mesh by splitting (instead of using CDT).\n");
#endif /* not CDT_ONLY */
  GNprintf("    -C  Check consistency of final mesh.\n");
#endif /* not REDUCED */
  GNprintf("    -Q  Quiet:  No terminal output except errors.\n");
  GNprintf("    -V  Verbose:  Detailed information on what I'm doing.\n");
  GNprintf("    -h  Help:  Detailed instructions for Triangle.\n");
  triexit(0);
}
...
void info()
{
  ...
  GNprintf(
"A Two-Dimensional Quality Mesh Generator and Delaunay Triangulator.\n");
  GNprintf("Version 1.6\n\n");
  GNprintf(
"Copyright 1993, 1995, 1997, 1998, 2002, 2005 Jonathan Richard Shewchuk\n");
  GNprintf("2360 Woolsey #H / Berkeley, California 94705-1927\n");
  GNprintf("Bugs/comments to jrs@cs.berkeley.edu\n");
  GNprintf(
"Created as part of the Quake project (tools for earthquake simulation).\n");
  GNprintf(
"Supported in part by NSF Grant CMS-9318163 and an NSERC 1967 Scholarship.\n");
  GNprintf("There is no warranty whatsoever.  Use at your own risk.\n");
#ifdef SINGLE
  GNprintf("This executable is compiled for single precision arithmetic.\n\n\n");
#else /* not SINGLE */
  GNprintf("This executable is compiled for double precision arithmetic.\n\n\n");
#endif /* not SINGLE */
  GNprintf(
...
"and possibly a .poly file and an .area file as well.  The formats of these\n"
  ...
  GNprintf("files are described below.\n\n");
  GNprintf("Command Line Switches:\n\n");
  GNprintf(
...
"        triunsuitable().  In either case, the -u switch causes the user-\n");
  GNprintf("        defined test to be applied to every triangle.\n");
  GNprintf(
...
"        .poly file to be written containing the boundary edges of the mesh\n"
  ...
  GNprintf("        (useful if no .poly file was read).\n");
  GNprintf(
...
"    -g  Outputs the mesh to an Object File Format (.off) file, suitable for\n"
  ...
  GNprintf("        viewing with the Geometry Center's Geomview package.\n");
  GNprintf(
...
"        to maintain constraining segments on later refinements of the mesh.\n"
  ...
  GNprintf("    -N  No output .node file.\n");
  GNprintf("    -E  No output .ele file.\n");
  GNprintf(
...
"        using a .node file for input, because no .node file is written, so\n"
  ...
  GNprintf("        there is no record of any added Steiner points.\n");
  GNprintf("    -O  No holes.  Ignores the holes in the .poly file.\n");
  GNprintf(
...
"        improvement in speed and creates the possibility that Triangle will\n"
  ...
  GNprintf("        fail to produce a valid mesh.  Not recommended.\n");
  GNprintf(
...
"        always adds enough vertices to ensure that all the segments of the\n"
  ...
  GNprintf("        PLSG are recovered, ignoring the limit if necessary.\n");
  GNprintf(
...
"        construct a Delaunay triangulation.  Try it if the divide-and-\n");
  GNprintf("        conquer algorithm fails.\n");
  GNprintf(
...
"        triangulation.  Warning:  does not use exact arithmetic for all\n");
  GNprintf("        calculations.  An exact result is not guaranteed.\n");
  GNprintf(
...
"        prints so much that Triangle runs much more slowly.  `-VVVV' gives\n"
  ...
  GNprintf("        information only a debugger could love.\n");
  GNprintf("    -h  Help:  Displays these instructions.\n");
  GNprintf("\n");
  GNprintf("Definitions:\n");
  GNprintf("\n");
  GNprintf(
...
"  inside the circumscribing circle (circle that passes through all three\n");
  GNprintf("  vertices) of any triangle in the triangulation.\n\n");
  GNprintf(
...
"  to some input vertex than to any other input vertex.  The Voronoi diagram\n"
  ...
  GNprintf("  is a geometric dual of the Delaunay triangulation.\n\n");
  GNprintf(
...
"  Segments may intersect each other only at their endpoints.  The file\n");
  GNprintf("  format for PSLGs (.poly files) is described below.\n\n");
  GNprintf(
...
"  cover the convex hull of the PSLG, or it might cover only a segment-\n");
  GNprintf("  bounded region (e.g. a polygon).\n\n");
  GNprintf(
...
"  The new vertices are frequently necessary to maintain the Delaunay\n");
  GNprintf("  property while ensuring that every segment is represented.\n\n");
  GNprintf(
...
"  triangulation of a PSLG whose triangles are constrained Delaunay.  New\n");
  GNprintf("  vertices may appear, and input segments may be subdivided into\n");
  GNprintf(
...
"  by the -q switch are usually CCDTs, but can be made conforming Delaunay\n");
  GNprintf("  with the -D switch.\n\n");
  GNprintf("File Formats:\n\n");
  GNprintf(
...
"  another program, use the -z switch if you wish to number objects from\n");
  GNprintf("  zero.)  Examples of these file formats are given below.\n\n");
  GNprintf("  .node files:\n");
  GNprintf(
...
"    -a, -u, -D, or -s is selected, each new Steiner point added to the mesh\n"
  ...
  GNprintf("    has attributes assigned to it by linear interpolation.\n\n");
  GNprintf(
...
"    .node file produced by Triangle contains boundary markers in the last\n");
  GNprintf("    column unless they are suppressed by the -B switch.\n\n");
  GNprintf("  .ele files:\n");
  GNprintf(
...
"    around each triangle.  (The remaining nodes, if any, depend on the type\n"
  ...
  GNprintf("    of finite element used.)\n\n");
  GNprintf(
...
"    among nearby triangles as the triangulation is refined.  Attributes do\n"
  ...
  GNprintf("    not diffuse across segments, so attributes used to identify\n");
  GNprintf("    segment-bounded regions remain intact.\n\n");
  GNprintf(
...
"    Following lines:  <vertex #> <x> <y> [attributes] [boundary marker]\n");
  GNprintf("    One line:  <# of segments> <# of boundary markers (0 or 1)>\n");
  GNprintf(
"    Following lines:  <segment #> <endpoint> <endpoint> [boundary marker]\n");
  GNprintf("    One line:  <# of holes>\n");
  GNprintf("    Following lines:  <hole #> <x> <y>\n");
  GNprintf(
...
"    means that you must include its endpoints in the vertex list.  Each\n");
  GNprintf("    segment, like each point, may have a boundary marker.\n\n");
  GNprintf(
...
"    segment itself is also eaten.  Do not place a hole directly on a\n");
  GNprintf("    segment; if you do, Triangle chooses one side of the segment\n");
  GNprintf("    arbitrarily.\n\n");
  GNprintf(
...
"    simultaneously and wish to assign an attribute to some region without\n");
  GNprintf("    imposing an area constraint, use a negative maximum area.\n\n");
  GNprintf(
...
"    the segment, and you'll have a needle-sharp triangle in your output--or\n"
  ...
  GNprintf("    a lot of tiny triangles if you're generating a quality mesh.\n");
  GNprintf("\n");
  GNprintf(
...
"    mesh, and don't want segments to be missing in later triangulations.\n");
  GNprintf("\n");
  GNprintf("  .area files:\n");
  GNprintf("    First line:  <# of triangles>\n");
  GNprintf("    Following lines:  <triangle #> <maximum area>\n");
  GNprintf("\n");
  GNprintf(
...
"    triangle may be left unconstrained by assigning it a negative maximum\n");
  GNprintf("    area.\n\n");
  GNprintf("  .edge files:\n");
  GNprintf("    First line:  <# of edges> <# of boundary markers (0 or 1)>\n");
  GNprintf(
...
"    infinite ray with only one endpoint.  For these edges, a different\n");
  GNprintf("    format is used:\n\n");
  GNprintf("        <edge #> <endpoint> -1 <direction x> <direction y>\n\n");
  GNprintf(
"    The `direction' is a floating-point vector that indicates the direction\n"
);
  GNprintf("    of the infinite ray.\n\n");
  GNprintf("  .neigh files:\n");
  GNprintf(
...
"  finite element mesh.  You can prevent boundary markers from being written\n"
  ...
  GNprintf("  into files produced by Triangle by using the -B switch.\n\n");
  GNprintf(
"  The boundary marker associated with each segment in an output .poly file\n"
);
  GNprintf("  and each edge in an output .edge file is chosen as follows:\n");
  GNprintf(
...
"      (even the boundary of a hole), then the edge is assigned the marker\n");
  GNprintf("      one (1).\n");
  GNprintf("    - Otherwise, the edge is assigned the marker zero (0).\n");
  GNprintf(
"  The boundary marker associated with each vertex in an output .node file\n");
  GNprintf("  is chosen as follows:\n");
  GNprintf(
...
"      vertex is assigned the same marker.  If the vertex lies on several\n");
  GNprintf("      such segments, one of the markers is chosen arbitrarily.\n");
  GNprintf(
"    - Otherwise, if the vertex occurs on a boundary of the triangulation,\n");
  GNprintf("      then the vertex is assigned the marker one (1).\n");
  GNprintf("    - Otherwise, the vertex is assigned the marker zero (0).\n");
  GNprintf("\n");
  GNprintf(
...
"  all) in your input files.  In the output files, all boundary vertices,\n");
  GNprintf("  edges, and segments will be assigned the value one.\n\n");
  GNprintf("Triangulation Iteration Numbers:\n\n");
  GNprintf(
...
"  triangulation, and output the files mesh.4.node, mesh.4.ele, and\n");
  GNprintf("  mesh.4.poly.  Files with no iteration number are treated as if\n");
  GNprintf(
...
"  points, a .node file is written.  This can be quite convenient for\n");
  GNprintf("  computing CDTs or quality meshes.)\n\n");
  GNprintf("Examples of How to Use Triangle:\n\n");
  GNprintf(
...
"  mesh to object.1.node and object.1.ele.  Each segment may be broken up\n");
  GNprintf("  into multiple subsegments; these are written to object.1.poly.\n");
  GNprintf("\n");
  GNprintf(
...
"    # A box with eight vertices in 2D, no attributes, one boundary marker.\n"
  ...
  GNprintf("    8 2 0 1\n");
  GNprintf("     # Outer box has these vertices:\n");
  GNprintf("     1   0 0   0\n");
  GNprintf("     2   0 3   0\n");
  GNprintf("     3   3 0   0\n");
  GNprintf("     4   3 3   33     # A special marker for this vertex.\n");
  GNprintf("     # Inner square has these vertices:\n");
  GNprintf("     5   1 1   0\n");
  GNprintf("     6   1 2   0\n");
  GNprintf("     7   2 1   0\n");
  GNprintf("     8   2 2   0\n");
  GNprintf("    # Five segments with boundary markers.\n");
  GNprintf("    5 1\n");
  GNprintf("     1   1 2   5      # Left side of outer box.\n");
  GNprintf("     # Square hole has these segments:\n");
  GNprintf("     2   5 7   0\n");
  GNprintf("     3   7 8   0\n");
  GNprintf("     4   8 6   10\n");
  GNprintf("     5   6 5   0\n");
  GNprintf("    # One hole in the middle of the inner square.\n");
  GNprintf("    1\n");
  GNprintf("     1   1.5 1.5\n");
  GNprintf("\n");
  GNprintf(
...
"  other vertices but 4 have been marked to indicate that they lie on a\n");
  ...
  GNprintf("      11    3   1.5    1\n");
  GNprintf("      12    1.5   3    1\n");
  GNprintf("    # Generated by triangle -pqc box.poly\n");
  GNprintf("\n");
  GNprintf("  Here is the output file `box.1.ele', with twelve triangles.\n");
  GNprintf("\n");
  GNprintf("    12  3  0\n");
  GNprintf("       1     5   6   9\n");
  GNprintf("       2    10   3   7\n");
...
"  newly added vertices.  Note also that <# of vertices> is set to zero to\n");
  GNprintf("  indicate that the vertices should be read from the .node file.\n");
  GNprintf("\n");
  GNprintf("    0  2  0  1\n");
  GNprintf("    12  1\n");
  GNprintf("       1     1   9     5\n");
  ...
  GNprintf("    # Generated by triangle -pqc box.poly\n");
  GNprintf("\n");
  GNprintf("Refinement and Area Constraints:\n");
  GNprintf("\n");
  GNprintf(
...
"  consider instead using the -u switch and writing a C procedure that\n");
  GNprintf("  determines which triangles are too large.\n\n");
  GNprintf(
...
"  the nodes at the corners of an element; the midpoint nodes on each edge\n");
  GNprintf("  are discarded before the mesh is refined.\n\n");
  GNprintf(
...
"  are typically based on a posteriori error estimates resulting from a\n");
  GNprintf("  finite element simulation on that mesh.\n\n");
  GNprintf(
...
"  x.2' creates a sequence of successively finer meshes x.1, x.2, and x.3,\n");
  GNprintf("  suitable for multigrid.\n\n");
  GNprintf("Convex Hulls and Mesh Boundaries:\n\n");
  GNprintf(
...
"  There are faster algorithms for finding a two-dimensional convex hull\n");
  GNprintf("  than triangulation, of course, but this one comes for free.\n\n");
  GNprintf(
...
"  If you also use the -p switch, the output .poly file contains all the\n");
  GNprintf("  segments from the input .poly file as well.\n\n");
  GNprintf("Voronoi Diagrams:\n\n");
  GNprintf(
...
"  rays.  (The choice of filenames makes it easy to run the set of Voronoi\n");
  GNprintf("  vertices through Triangle, if so desired.)\n\n");
  GNprintf(
...
"  construct a conforming Delaunay triangulation instead of a CCDT, so the\n");
  GNprintf("  Voronoi diagram will be valid.\n\n");
  GNprintf("Mesh Topology:\n\n");
  GNprintf(
...
"  output files with the recollection that the Delaunay triangulation and\n");
  GNprintf("  the Voronoi diagram are planar duals.\n\n");
  GNprintf(
...
"  vertex j of the corresponding .v.node file.  Voronoi cell k is the dual\n");
  GNprintf("  of vertex k of the corresponding .node file.\n\n");
  GNprintf(
...
"  respectively.  To find which Voronoi cells are adjacent to each other,\n");
  GNprintf("  just read the list of Delaunay edges.\n\n");
  GNprintf(
...
"  Voronoi vertices attached to it.  You might find this more convenient\n");
  GNprintf("  than the .v.edge file.\n\n");
  GNprintf("Quadratic Elements:\n\n");
  GNprintf(
...
"  and sixth nodes appearing opposite the first, second, and third corners\n");
  GNprintf("  respectively.\n\n");
  GNprintf("Domains with Small Angles:\n\n");
  GNprintf(
...
"  is reduced to memory precision.  Triangle does this if it is compiled\n");
  GNprintf("  correctly.  See the makefile for details.\n\n");
  GNprintf(
...
"  inputs most likely to cause errors if you use the -X switch.  Hence, the\n"
  ...
  GNprintf("  -X switch is not recommended.\n\n");
  GNprintf(
...
"  files.)  Similarly, exact arithmetic is not used to compute the vertices\n"
  ...
  GNprintf("  of the Voronoi diagram.\n\n");
  GNprintf(
...
"  being performed exactly, while overflow typically causes a floating\n");
  GNprintf("  exception.\n\n");
  GNprintf("Calling Triangle from Another Program:\n\n");
  GNprintf("  Read the file triangle.h for details.\n\n");
  GNprintf("Troubleshooting:\n\n");
  GNprintf("  Please read this section before mailing me bugs.\n\n");
  GNprintf("  `My output mesh has no triangles!'\n\n");
  GNprintf(
...
"    have placed a hole badly, thereby eating all your triangles.  To test\n");
  GNprintf("    these possibilities, try again with the -c and -O switches.\n");
  GNprintf(
"    Alternatively, all your input vertices may be collinear, in which case\n"
);
  GNprintf("    you can hardly expect to triangulate them.\n\n");
  GNprintf("  `Triangle doesn't terminate, or just crashes.'\n\n");
  GNprintf(
...
"    between 6247133 and 6247134, you're not leaving much floating-point\n");
  GNprintf("    precision for Triangle to work with.\n\n");
  GNprintf(
...
"    above.)  Again, I recommend compiling Triangle for double (rather\n");
  GNprintf("    than single) precision arithmetic.\n\n");
  GNprintf(
...
"    extremely tiny triangles, or it might fail because of insufficient\n");
  GNprintf("    floating-point precision.\n\n");
  GNprintf(
...
"    don't use the -X switch.  If you still have problems, please report the\n"
  ...
  GNprintf(
"  `Triangle executes without incident, but when I look at the resulting\n");
  GNprintf("  Voronoi diagram, it has overlapping edges or other geometric\n");
  GNprintf("  inconsistencies.'\n");
  GNprintf("\n");
  GNprintf(
...
"    holes, and you use the -D switch to construct a conforming Delaunay\n");
  GNprintf("    triangulation (instead of a CDT or CCDT).\n\n");
  GNprintf(
...
"  doesn't, and you'll have to connect each adjacent pair of vertices with a\n"
  ...
  GNprintf("  separate segment.  If you don't like it, tough.\n\n");
  GNprintf(
...
"  endpoints, try not to let the intersections fall extremely close to PSLG\n"
  ...
  GNprintf("  vertices or each other.\n\n");
  GNprintf(
...
"  triangles constrained Delaunay?  Triangle's Delaunay refinement algorithm\n"
  ...
  GNprintf("  assumes that it starts with a CDT.\n\n");
  GNprintf("Show Me:\n\n");
  GNprintf(
...
"  you have the X Windows system.  Sorry, Microsoft Windows users.\n");
  GNprintf("\n");
  GNprintf("Triangle on the Web:\n");
  GNprintf("\n");
  GNprintf("  To see an illustrated version of these instructions, check out\n");
  GNprintf("\n");
  GNprintf("    http://www.cs.cmu.edu/~quake/triangle.html\n");
  GNprintf("\n");
  GNprintf("A Brief Plea:\n");
  GNprintf("\n");
  GNprintf(
...
"  you.  Also, I can put you on a list to receive email whenever a new\n");
  GNprintf("  version of Triangle is available.\n\n");
  GNprintf(
...
"  Schachter, Micha Sharir, Peter W. Shor, Daniel D. Sleator, Jorge Stolfi,\n"
  ...
  GNprintf("  Robert E. Tarjan, Alper Ungor, Christopher J. Van Wyk, Noel J.\n");
  GNprintf(
"  Walkington, and Binhai Zhu.  See the comments at the beginning of the\n");
  GNprintf("  source code for references.\n\n");
  triexit(0);
}
...
void internalerror()
{
  //GNprintf("  Please report this bug to jrs@cs.berkeley.edu\n");
  //GNprintf("  Include the message above, your input data set, and the exact\n");
  //GNprintf("    command line you used to run Triangle.\n");
  triexit(1);
}
...
#define STARTINDEX 1
      ...
      for (j = STARTINDEX; argv[i][j] != '\0'; j++) {
        ...
        if (argv[i][j] == 'a') {
          ...
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            if (b->maxarea <= 0.0) {
              GNprintf("Error:  Maximum area must be greater than zero.\n");
              triexit(1);
	    }
	  } else {
            b->vararea = 1;
  ...
  if (b->weighted && (b->poly || b->quality)) {
    b->weighted = 0;
    if (!b->quiet) {
      GNprintf("Warning:  weighted triangulations (-w, -W) are incompatible\n");
      GNprintf("  with PSLGs (-p) and meshing (-q, -a, -u).  Weights ignored.\n"
             );
    }
  }
  if (b->jettison && b->nonodewritten && !b->quiet) {
    GNprintf("Warning:  -j and -N switches are somewhat incompatible.\n");
    GNprintf("  If any vertices are jettisoned, you will need the output\n");
    GNprintf("  .node file to reconstruct the new node indices.");
  }
  ...
  if (b->noiterationnum) {
    ...
    sGNprintf(b->outnodefilename, workstring, meshnumber + 1);
...

{
  ...
  GNprintf("triangle x%lx with orientation %d:\n", (unsigned long) t->tri,
         t->orient);
  decode(t->tri[0], printtri);
  if (printtri.tri == m->dummytri) {
    GNprintf("    [0] = Outer space\n");
  } else {
  ...
  if (printvertex == (vertex) NULL)
    GNprintf("    Origin[%d] = NULL\n", (t->orient + 1) % 3 + 3);
  else
    GNprintf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 1) % 3 + 3, (unsigned long) printvertex,
           printvertex[0], printvertex[1]);
  dest(*t, printvertex);
  if (printvertex == (vertex) NULL)
    GNprintf("    Dest  [%d] = NULL\n", (t->orient + 2) % 3 + 3);
  else
    GNprintf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 2) % 3 + 3, (unsigned long) printvertex,
           printvertex[0], printvertex[1]);
  apex(*t, printvertex);
  if (printvertex == (vertex) NULL)
    GNprintf("    Apex  [%d] = NULL\n", t->orient + 3);
  else
    GNprintf("    Apex  [%d] = x%lx  (%.12g, %.12g)\n",
...

{
  ...
  GNprintf("subsegment x%lx with orientation %d and mark %d:\n",
         (unsigned long) s->ss, s->ssorient, mark(*s));
  sdecode(s->ss[0], printsh);
  if (printsh.ss == m->dummysub) {
    GNprintf("    [0] = No subsegment\n");
  } else {
    GNprintf("    [0] = x%lx  %d\n", (unsigned long) printsh.ss,
           printsh.ssorient);
  }
  ...
  if (printvertex == (vertex) NULL)
    GNprintf("    Segment origin[%d] = NULL\n", 4 + s->ssorient);
  else
    GNprintf("    Segment origin[%d] = x%lx  (%.12g, %.12g)\n",
           4 + s->ssorient, (unsigned long) printvertex,
           printvertex[0], printvertex[1]);
  segdest(*s, printvertex);
  if (printvertex == (vertex) NULL)
    GNprintf("    Segment dest  [%d] = NULL\n", 5 - s->ssorient);
  else
    GNprintf("    Segment dest  [%d] = x%lx  (%.12g, %.12g)\n",
...

{
  ...
  b->noexact = 0;
  if (!b->quiet) {
    GNprintf("  Checking consistency of mesh...\n");
  }
  horrors = 0;
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if (oppotri.tri != m->dummytri) {
        ...
        if ((triangleloop.tri != oppooppotri.tri)
          ...
          GNprintf("  !! !! Asymmetric triangle-triangle bond:\n");
          if (triangleloop.tri == oppooppotri.tri) {
            GNprintf("   (Right triangle, wrong orientation)\n");
          }
          GNprintf("    First ");
          printtriangle(m, b, &triangleloop);
          GNprintf("    Second (nonreciprocating) ");
        ...
        if ((triorg != oppodest) || (tridest != oppoorg)) {
          GNprintf("  !! !! Mismatched edge coordinates between two triangles:\n"
                 );
          GNprintf("    First mismatched ");
          printtriangle(m, b, &triangleloop);
          GNprintf("    Second mismatched ");
          printtriangle(m, b, &oppotri);
          horrors++;
        }
      }
  ...
  if (horrors == 0) {
    if (!b->quiet) {
      GNprintf("  In my studied opinion, the mesh appears to be consistent.\n");
    }
  } else if (horrors == 1) {
    GNprintf("  !! !! !! !! Precisely one festering wound discovered.\n");
  } else {
    GNprintf("  !! !! !! !! %d abominations witnessed.\n", horrors);
  }
...

{
  ...
  b->noexact = 0;
  if (!b->quiet) {
    GNprintf("  Checking Delaunay property of mesh...\n");
  }
  horrors = 0;
  /* Run through the list of triangles, checking each one. */
  traversalinit(&m->triangles);
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if (shouldbedelaunay) {
        if (nonregular(m, b, triorg, tridest, triapex, oppoapex) > 0.0) {
          if (!b->weighted) {
            GNprintf("  !! !! Non-Delaunay pair of triangles:\n");
            GNprintf("    First non-Delaunay ");
            printtriangle(m, b, &triangleloop);
            GNprintf("    Second non-Delaunay ");
          } else {
            GNprintf("  !! !! Non-regular pair of triangles:\n");
            GNprintf("    First non-regular ");
            printtriangle(m, b, &triangleloop);
            GNprintf("    Second non-regular ");
          }
          printtriangle(m, b, &oppotri);
          horrors++;
        }
  ...
  "  By virtue of my perceptive intelligence, I declare the mesh Delaunay.\n");
    ...
    GNprintf("  !! !! !! !! %d obscenities viewed with horror.\n", horrors);
  }
  /* Restore the status of exact arithmetic. */
  b->noexact = saveexact;
}
...

{
  ...
  if (b->verbose > 2) {
    GNprintf("  Queueing bad triangle:\n");
    GNprintf("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
...

{
  ...
  if (b->verbose) {
    GNprintf("    Constructing mapping from vertices to triangles.\n");
  }
  traversalinit(&m->triangles);
  triangleloop.tri = triangletraverse(m);
  while (triangleloop.tri != (triangle *) NULL) {
...

{
  ...
  if (b->verbose > 2) {
    GNprintf("  Searching for point (%.12g, %.12g).\n",
  ...
  while (1) {
    if (b->verbose > 2) {
      GNprintf("    At (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
...

{
  ...
  if (b->verbose > 2) {
    GNprintf("  Randomly sampling for a triangle near point (%.12g, %.12g).\n",
           searchpoint[0], searchpoint[1]);
  }
  ...
  if (b->verbose > 2) {
    GNprintf("    Boundary triangle has origin (%.12g, %.12g).\n",
           torg[0], torg[1]);
  }
  ...
  if (m->recenttri.tri != (triangle *) NULL) {
    if (!deadtri(m->recenttri.tri)) {
      ...
      if (dist < searchdist) {
        ...
        searchdist = dist;
        if (b->verbose > 2) {
          GNprintf("    Choosing recent triangle with origin (%.12g, %.12g).\n",
                 torg[0], torg[1]);
        }
      }
    }
    ...
    do {
      ...
      if (!deadtri(sampletri.tri)) {
        ...
        if (dist < searchdist) {
          ...
          searchdist = dist;
          if (b->verbose > 2) {
            GNprintf("    Choosing triangle with origin (%.12g, %.12g).\n",
                   torg[0], torg[1]);
          }
        }
      }
...

{
  ...
  if (top.tri == m->dummytri) {
    GNprintf("Internal error in flip():  Attempt to flip on boundary.\n");
    lnextself(*flipedge);
    return;
  }
  if (m->checksegments) {
    ...
    if (toplsubseg.ss != m->dummysub) {
      GNprintf("Internal error in flip():  Attempt to flip a segment.\n");
      lnextself(*flipedge);
      return;
    }
  }
  ...
  if (b->verbose > 2) {
    GNprintf("  Edge flip results in left ");
...

{
...
#ifdef SELF_CHECK
  if (top.tri == m->dummytri) {
    GNprintf("Internal error in unflip():  Attempt to flip on boundary.\n");
    lnextself(*flipedge);
    return;
  }
  if (m->checksegments) {
    tspivot(*flipedge, toplsubseg);
    if (toplsubseg.ss != m->dummysub) {
      GNprintf("Internal error in unflip():  Attempt to flip a subsegment.\n");
      lnextself(*flipedge);
      return;
    }
  }
  ...
  if (b->verbose > 2) {
    GNprintf("  Edge unflip results in left ");
    printtriangle(m, b, flipedge);
    GNprintf("  and right ");
    printtriangle(m, b, &top);
  }
...

{
  ...
  if ((intersect == ONEDGE) || (intersect == OUTSIDE)) {
    ...
    if (counterclockwise(m, b, rightvertex, leftvertex, botvertex) < 0.0) {
      GNprintf("Internal error in insertvertex():\n");
      GNprintf(
    ...
    if (mirrorflag) {
      if (counterclockwise(m, b, leftvertex, rightvertex, topvertex) < 0.0) {
        GNprintf("Internal error in insertvertex():\n");
        GNprintf("  Clockwise triangle prior to edge vertex insertion (top).\n");
      }
...
#endif /* SELF_CHECK */
    if (b->verbose > 2) {
      GNprintf("  Updating bottom left ");
      printtriangle(m, b, &botright);
      if (mirrorflag) {
        GNprintf("  Updating top left ");
        printtriangle(m, b, &topright);
        GNprintf("  Creating top right ");
        printtriangle(m, b, &newtopright);
      }
      GNprintf("  Creating bottom right ");
    ...
    if (counterclockwise(m, b, rightvertex, leftvertex, botvertex) < 0.0) {
      GNprintf("Internal error in insertvertex():\n");
      GNprintf("  Clockwise triangle prior to vertex insertion.\n");
    }
    if (counterclockwise(m, b, rightvertex, leftvertex, newvertex) < 0.0) {
      GNprintf("Internal error in insertvertex():\n");
      GNprintf("  Clockwise triangle after vertex insertion (top).\n");
    }
    if (counterclockwise(m, b, leftvertex, botvertex, newvertex) < 0.0) {
      GNprintf("Internal error in insertvertex():\n");
      GNprintf("  Clockwise triangle after vertex insertion (left).\n");
    }
    if (counterclockwise(m, b, botvertex, rightvertex, newvertex) < 0.0) {
      GNprintf("Internal error in insertvertex():\n");
      GNprintf("  Clockwise triangle after vertex insertion (right).\n");
    }
#endif /* SELF_CHECK */
    if (b->verbose > 2) {
      GNprintf("  Updating top ");
      printtriangle(m, b, &horiz);
      GNprintf("  Creating left ");
      printtriangle(m, b, &newbotleft);
      GNprintf("  Creating right ");
      printtriangle(m, b, &newbotright);
    }
  }
        ...
        if (doflip) {
          ...
          if (newvertex != (vertex) NULL) {
            if (counterclockwise(m, b, leftvertex, newvertex, rightvertex) <
                0.0) {
              GNprintf("Internal error in insertvertex():\n");
              GNprintf("  Clockwise triangle prior to edge flip (bottom).\n");
            }
...
/*
            if (counterclockwise(m, b, rightvertex, farvertex, leftvertex) <
                0.0) {
              GNprintf("Internal error in insertvertex():\n");
              GNprintf("  Clockwise triangle prior to edge flip (top).\n");
            }
*/
            if (counterclockwise(m, b, farvertex, leftvertex, newvertex) <
                0.0) {
              GNprintf("Internal error in insertvertex():\n");
              GNprintf("  Clockwise triangle after edge flip (left).\n");
            }
            if (counterclockwise(m, b, newvertex, rightvertex, farvertex) <
                0.0) {
              GNprintf("Internal error in insertvertex():\n");
              GNprintf("  Clockwise triangle after edge flip (right).\n");
            }
          }
#endif /* SELF_CHECK */
          if (b->verbose > 2) {
...

{
  ...
  if (b->verbose > 2) {
    GNprintf("  Triangulating interior polygon at edge\n");
    GNprintf("    (%.12g, %.12g) (%.12g, %.12g)\n", leftbasevertex[0],
  ...
  if (b->verbose > 2) {
    GNprintf("    Connecting edge to (%.12g, %.12g)\n", bestvertex[0],
           bestvertex[1]);
  }
  if (bestnumber > 1) {
...

{
  ...
  while (!otriequal(*deltri, countingtri)) {
    ...
    if (countingtri.tri == m->dummytri) {
      GNprintf("Internal error in deletevertex():\n");
      GNprintf("  Attempt to delete boundary vertex.\n");
      internalerror();
    }
#endif /* SELF_CHECK */
    edgecount++;
...

{
  ...
  if (b->verbose > 2) {
    GNprintf("  Creating base bounding ");
  ...
  while (1) {
    ...
    if (leftfinished && rightfinished) {
      ...
      if (b->verbose > 2) {
        GNprintf("  Creating top bounding ");
...

{
  ...
  if (b->verbose > 2) {
    GNprintf("  Triangulating %d vertices.\n", vertices);
  }
  if (vertices == 2) {
    ...
    divconqrecurse(m, b, &sortarray[divider], vertices - divider, 1 - axis,
                   &innerright, farright);
    if (b->verbose > 1) {
      GNprintf("  Joining triangulations with %d and %d vertices.\n", divider,
             vertices - divider);
    }
...

{
  ...
  if (b->verbose) {
    GNprintf("  Removing ghost triangles.\n");
  }
...

{
  ...
  for (j = 1; j < m->invertices; j++) {
    if ((sortarray[i][0] == sortarray[j][0])
      ...
      if (!b->quiet) {
			//char mm[256];
			//sprintf(mm, "Warning:  A duplicate vertex at (%.12g, %.12g) appeared and was ignored.\n",
...
"Warning:  A duplicate vertex at (%.12g, %.12g) appeared and was ignored.\n",
  ...
  if (b->verbose) {
    GNprintf("  Forming triangulation.\n");
  }
...

{
  ...
  if (b->verbose) {
    GNprintf("  Creating triangular bounding box.\n");
  }
...

{
  ...
  if (b->verbose) {
    GNprintf("  Removing triangular bounding box.\n");
  }
  /* Find a boundary triangle. */
...

{
  ...
  boundingbox(m, b);
  if (b->verbose) {
    GNprintf("  Incrementally inserting vertices.\n");
  }
  traversalinit(&m->vertices);
...

{
  ...
  if (b->verbose) {
    GNprintf("  Placing vertices in event heap.\n");
  }
  ...
  do {
    if (heapsize == 0) {
      GNprintf("Error:  Input vertices are all identical.\n");
      triexit(1);
    }
...
#else /* not REDUCED */
  if (!b->quiet) {
    GNprintf("Constructing Delaunay triangulation ");
    if (b->incremental) {
      GNprintf("by incremental method.\n");
    } else if (b->sweepline) {
      GNprintf("by sweepline method.\n");
    } else {
      GNprintf("by divide-and-conquer method.\n");
    }
  }
  if (b->incremental) {
...

{
  ...
  incorners = corners;
  if (incorners < 3) {
    GNprintf("Error:  Triangles must have at least 3 vertices.\n");
    triexit(1);
  }
  m->eextras = attribs;
#else /* not TRILIBRARY */
  ...
  if (elefile == (FILE *) NULL) {
    GNprintf("  Error:  Cannot access file %s.\n", elefilename);
    triexit(1);
  }
  ...
  if (*stringptr == '\0') {
    ...
    if (incorners < 3) {
      GNprintf("Error:  Triangles in %s must have at least 3 vertices.\n",
             elefilename);
      triexit(1);
    }
  }
  ...
  if (b->vararea) {
    ...
    if (areaelements != m->inelements) {
      GNprintf("Error:  %s and %s disagree on number of triangles.\n",
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      if ((corner[j] < b->firstnumber) ||
        ...
        GNprintf("Error:  Triangle %ld has an invalid vertex index.\n",
               elementnumber);
        triexit(1);
      }
    }
    ...
    for (j = 0; j < 3; j++) {
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        GNprintf("Error:  Triangle %ld is missing vertex %d in %s.\n",
  ...
  if (b->poly) {
    if (b->verbose) {
      GNprintf("  Marking segments in triangulation.\n");
    }
    ...
    while (subsegloop.ss != (subseg *) NULL) {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        GNprintf("Error:  Segment %ld has no endpoints in %s.\n", segmentnumber,
               polyfilename);
        triexit(1);
      } else {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        GNprintf("Error:  Segment %ld is missing its second endpoint in %s.\n",
      ...
      for (j = 0; j < 2; j++) {
        if ((end[j] < b->firstnumber) ||
          ...
          GNprintf("Error:  Segment %ld has an invalid vertex index.\n", 
                 segmentnumber);
          triexit(1);
        }
      }
...

{
  ...
  while (leftflag) {
    ...
    onextself(*searchtri);
    if (searchtri->tri == m->dummytri) {
      GNprintf("Internal error in finddirection():  Unable to find a\n");
      GNprintf("  triangle leading from (%.12g, %.12g) to", startvertex[0],
             startvertex[1]);
      GNprintf("  (%.12g, %.12g).\n", searchpoint[0], searchpoint[1]);
      internalerror();
    }
...

{
  ...
  denom = ty * ex - tx * ey;
  if (denom == 0.0) {
    GNprintf("Internal error in segmentintersection():");
    GNprintf("  Attempt to find intersection of parallel segments.\n");
    internalerror();
  }
  ...
  if (success != SUCCESSFULVERTEX) {
    GNprintf("Internal error in segmentintersection():\n");
    GNprintf("  Failure to split a segment.\n");
    internalerror();
  }
  ...
  if ((leftvertex[0] == endpoint1[0]) && (leftvertex[1] == endpoint1[1])) {
    ...
    GNprintf("Internal error in segmentintersection():\n");
    GNprintf("  Topological inconsistency after splitting a segment.\n");
    internalerror();
  }
...

{
  ...
  if (b->verbose > 2) {
    GNprintf("Forcing segment into triangulation by recursive splitting:\n");
  ...
  if (success == DUPLICATEVERTEX) {
    if (b->verbose > 2) {
      GNprintf("  Segment intersects existing vertex (%.12g, %.12g).\n",
             newvertex[0], newvertex[1]);
    }
    ...
    if (success == VIOLATINGVERTEX) {
      if (b->verbose > 2) {
        GNprintf("  Two segments intersect at (%.12g, %.12g).\n",
               newvertex[0], newvertex[1]);
      }
      ...
      success = insertvertex(m, b, newvertex, &searchtri1, &brokensubseg,
                             0, 0);
      if (success != SUCCESSFULVERTEX) {
        GNprintf("Internal error in conformingedge():\n");
        GNprintf("  Failure to split a segment.\n");
        internalerror();
      }
    }
...

{
  ...
  if (b->verbose > 1) {
    GNprintf("  Connecting (%.12g, %.12g) to (%.12g, %.12g).\n",
  ...
  if (checkvertex != endpoint1) {
    ...
    if (locate(m, b, endpoint1, &searchtri1) != ONVERTEX) {
      ...
      GNprintf("  (%.12g, %.12g) in triangulation.\n",
             endpoint1[0], endpoint1[1]);
      internalerror();
    }
  }
...

{
  ...
  if (b->poly) {
    if (!b->quiet) {
      GNprintf("Recovering segments in Delaunay triangulation.\n");
    }
#ifdef TRILIBRARY
    ...
    if (m->insegments > 0) {
      makevertexmap(m, b);
      if (b->verbose) {
        GNprintf("  Recovering PSLG segments.\n");
      }
    }
    ...
    for (i = 0; i < m->insegments; i++) {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        GNprintf("Error:  Segment %d has no endpoints in %s.\n",
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        GNprintf("Error:  Segment %d is missing its second endpoint in %s.\n",
               b->firstnumber + i, polyfilename);
        triexit(1);
      } else {
      ...
      if ((end1 < b->firstnumber) ||
        ...
        if (!b->quiet) {
          GNprintf("Warning:  Invalid first endpoint of segment %d in %s.\n",
                 b->firstnumber + i, polyfilename);
        }
      } else if ((end2 < b->firstnumber) ||
                 (end2 >= b->firstnumber + m->invertices)) {
        if (!b->quiet) {
          GNprintf("Warning:  Invalid second endpoint of segment %d in %s.\n",
                 b->firstnumber + i, polyfilename);
        }
      } else {
        ...
        if ((endpoint1[0] == endpoint2[0]) && (endpoint1[1] == endpoint2[1])) {
          if (!b->quiet) {
            GNprintf("Warning:  Endpoints of segment %d are coincident in %s.\n",
                   b->firstnumber + i, polyfilename);
          }
        } else {
  ...
  if (b->convex || !b->poly) {
    ...
    if (b->verbose) {
      GNprintf("  Enclosing convex hull with segments.\n");
    }
    markhull(m, b);
  }
}
...

{
  ...
  if (b->verbose) {
    GNprintf("  Marking concavities (external triangles) for elimination.\n");
  }
  /* Find a triangle handle on the hull. */
  hulltri.tri = m->dummytri;
  hulltri.orient = 0;
...

{
  ...
  if (b->verbose) {
    GNprintf("  Marking neighbors of marked triangles.\n");
  }
  ...
  while (virusloop != (triangle **) NULL) {
    ...
    if (b->verbose > 2) {
      ...
      GNprintf("    Checking (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
  ...
  if (b->verbose) {
    GNprintf("  Deleting marked triangles.\n");
  }
  ...
  while (virusloop != (triangle **) NULL) {
    ...
    for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
      ...
      if (testvertex != (vertex) NULL) {
        ...
        if (killorg) {
          if (b->verbose > 1) {
            GNprintf("    Deleting vertex (%.12g, %.12g)\n",
...

{
    ...
    for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
      ...
      if ((neighbor.tri != m->dummytri) && !infected(neighbor)
        ...
        if (b->verbose > 2) {
          ...
          GNprintf("    Marking (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
                 regionorg[0], regionorg[1], regiondest[0], regiondest[1],
                 regionapex[0], regionapex[1]);
        }
        /* Infect the neighbor. */
  ...
  if (b->verbose > 1) {
    GNprintf("  Unmarking marked triangles.\n");
  }
  traversalinit(&m->viri);
  virusloop = (triangle **) traverse(&m->viri);
  while (virusloop != (triangle **) NULL) {
...

{
  ...
  if (!(b->quiet || (b->noholes && b->convex))) {
    GNprintf("Removing unwanted triangles.\n");
    if (b->verbose && (holes > 0)) {
      GNprintf("  Marking holes for elimination.\n");
    }
  }
  ...
  if (regions > 0) {
    if (!b->quiet) {
      if (b->regionattrib) {
        if (b->vararea) {
          GNprintf("Spreading regional attributes and area constraints.\n");
        } else {
          GNprintf("Spreading regional attributes.\n");
        }
      } else { 
        GNprintf("Spreading regional area constraints.\n");
      }
    }
...
void precisionerror()
{
  GNprintf("Try increasing the area criterion and/or reducing the minimum\n");
  GNprintf("  allowable angle so that tiny triangles are not created.\n");
#ifdef SINGLE
  GNprintf("Alternatively, try recompiling me with double precision\n");
  GNprintf("  arithmetic (by removing \"#define SINGLE\" from the\n");
  GNprintf("  source file or \"-DSINGLE\" from the makefile).\n");
#endif /* SINGLE */
}
...

{
  ...
  "  Splitting subsegment (%.12g, %.12g) (%.12g, %.12g) at (%.12g, %.12g).\n",
        ...
        if (((newvertex[0] == eorg[0]) && (newvertex[1] == eorg[1])) ||
          ...
          GNprintf("Error:  Ran out of precision at (%.12g, %.12g).\n",
                 newvertex[0], newvertex[1]);
          GNprintf("I attempted to split a segment to a smaller size than\n");
          GNprintf("  can be accommodated by the finite precision of\n");
          GNprintf("  floating point arithmetic.\n");
          precisionerror();
          triexit(1);
        }
        ...
        if ((success != SUCCESSFULVERTEX) && (success != ENCROACHINGVERTEX)) {
          GNprintf("Internal error in splitencsegs():\n");
          GNprintf("  Failure to split a segment.\n");
          internalerror();
        }
        if (m->steinerleft > 0) {
...

{
  ...
  if (b->verbose) {
    GNprintf("  Making a list of bad triangles.\n");
  }
  traversalinit(&m->triangles);
  triangleloop.orient = 0;
  triangleloop.tri = triangletraverse(m);
...

{
  ...
  if (!deadtri(badotri.tri) && (borg == badtri->triangorg) &&
    ...
    if (b->verbose > 1) {
      GNprintf("  Splitting this triangle at its circumcenter:\n");
      GNprintf("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n", borg[0],
    ...
    if (errorflag) {
      if (b->verbose) {
        GNprintf("  The new vertex is at the circumcenter of triangle\n");
        GNprintf("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
               borg[0], borg[1], bdest[0], bdest[1], bapex[0], bapex[1]);
      }
      GNprintf("This probably means that I am trying to refine triangles\n");
      GNprintf("  to a smaller size than can be accommodated by the finite\n");
      GNprintf("  precision of floating point arithmetic.  (You can be\n");
      GNprintf("  sure of this if I fail to terminate.)\n");
      precisionerror();
    }
  }
}
...

{
  ...
  if (!b->quiet) {
    GNprintf("Adding Steiner points to enforce quality.\n");
  }
  /* Initialize the pool of encroached subsegments. */
  poolinit(&m->badsubsegs, sizeof(struct badsubseg), BADSUBSEGPERBLOCK,
           BADSUBSEGPERBLOCK, 0);
  if (b->verbose) {
    GNprintf("  Looking for encroached subsegments.\n");
  }
  /* Test all segments to see if they're encroached. */
  tallyencs(m, b);
  if (b->verbose && (m->badsubsegs.items > 0)) {
    GNprintf("  Splitting encroached subsegments.\n");
  }
  ...
  if ((b->minangle > 0.0) || b->vararea || b->fixedarea || b->usertest) {
    ...
    m->checkquality = 1;
    if (b->verbose) {
      GNprintf("  Splitting bad triangles.\n");
    }
  ...
  if (!b->quiet && b->conformdel && (m->badsubsegs.items > 0) &&
    ...
    if (m->badsubsegs.items == 1) {
      GNprintf("  one encroached subsegment, and therefore might not be truly\n"
             );
    } else {
      GNprintf("  %ld encroached subsegments, and therefore might not be truly\n"
             , m->badsubsegs.items);
    }
    GNprintf("  Delaunay.  If the Delaunay property is important to you,\n");
    GNprintf("  try increasing the number of Steiner points (controlled by\n");
    GNprintf("  the -S switch) slightly and try again.\n\n");
  }
}
...

{
  ...
  if (!b->quiet) {
    GNprintf("Adding vertices for second-order triangles.\n");
  }
...

{
  ...
  do {
    ...
    if (result == (char *) NULL) {
      GNprintf("  Error:  Unexpected end of file in %s.\n", infilename);
      triexit(1);
    }
...

{
  ...
  if (m->invertices < 3) {
    GNprintf("Error:  Input must have at least three input vertices.\n");
    triexit(1);
  }
  if (m->mesh_dim != 2) {
    GNprintf("Error:  Triangle only works with two-dimensional meshes.\n");
    triexit(1);
  }
  if (m->nextras == 0) {
    b->weighted = 0;
  ...
  for (i = 0; i < m->invertices; i++) {
    ...
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      GNprintf("Error:  Vertex %d has no x coordinate.\n", b->firstnumber + i);
      triexit(1);
    }
    x = (REAL) strtod(stringptr, &stringptr);
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      GNprintf("Error:  Vertex %d has no y coordinate.\n", b->firstnumber + i);
      triexit(1);
    }
  ...
  if (*holes > 0) {
    ...
    *hlist = holelist;
    for (i = 0; i < 2 * *holes; i += 2) {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        GNprintf("Error:  Hole %d has no x coordinate.\n",
               b->firstnumber + (i >> 1));
        triexit(1);
      } else {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        GNprintf("Error:  Hole %d has no y coordinate.\n",
               b->firstnumber + (i >> 1));
        triexit(1);
      } else {
    ...
    if (*regions > 0) {
      ...
      *rlist = regionlist;
      ...
      for (i = 0; i < *regions; i++) {
        ...
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
          GNprintf("Error:  Region %d has no x coordinate.\n",
                 b->firstnumber + i);
          triexit(1);
        } else {
        ...
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
          GNprintf("Error:  Region %d has no y coordinate.\n",
                 b->firstnumber + i);
          triexit(1);
        } else {
          regionlist[index++] = (REAL) strtod(stringptr, &stringptr);
...

{
  ...
  fGNprintf(outfile, "# Generated by");
  for (i = 0; i < argc; i++) {
    fGNprintf(outfile, " ");
...

{
  ...
  if (outfile == (FILE *) NULL) {
    GNprintf("  Error:  Cannot create file %s.\n", nodefilename);
    triexit(1);
  }
  /* Number of vertices, number of dimensions, number of vertex attributes, */
  /*   and number of boundary markers (zero or one).                        */
  fGNprintf(outfile, "%ld  %d  %d  %d\n", outvertices, m->mesh_dim,
  ...
  while (vertexloop != (vertex) NULL) {
    if (!b->jettison || (vertextype(vertexloop) != UNDEADVERTEX)) {
      ...
      fGNprintf(outfile, "%4d    %.17g  %.17g", vertexnumber, vertexloop[0],
              vertexloop[1]);
      for (i = 0; i < m->nextras; i++) {
        /* Write an attribute. */
        fGNprintf(outfile, "  %.17g", vertexloop[i + 2]);
      }
      if (b->nobound) {
        fGNprintf(outfile, "\n");
      } else {
...

{
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    if (b->order == 1) {
      ...
      fGNprintf(outfile, "%4ld    %4d  %4d  %4d", elementnumber,
      ...
      fGNprintf(outfile, "%4ld    %4d  %4d  %4d  %4d  %4d  %4d", elementnumber,
...
#else /* not TRILIBRARY */
    for (i = 0; i < m->eextras; i++) {
      fGNprintf(outfile, "  %.17g", elemattribute(triangleloop, i));
    }
    fGNprintf(outfile, "\n");
#endif /* not TRILIBRARY */
...

{
  ...
  /*   and number of boundary markers (zero or one).                    */
  fGNprintf(outfile, "%d  %d  %d  %d\n", 0, m->mesh_dim, m->nextras,
          1 - b->nobound);
  /* Number of segments, number of boundary markers (zero or one). */
  fGNprintf(outfile, "%ld  %d\n", m->subsegs.items, 1 - b->nobound);
#endif /* not TRILIBRARY */
  ...
  while (subsegloop.ss != (subseg *) NULL) {
    ...
    if (b->nobound) {
      fGNprintf(outfile, "%4ld    %4d  %4d\n", subsegnumber,
              vertexmark(endpoint1), vertexmark(endpoint2));
    } else {
      fGNprintf(outfile, "%4ld    %4d  %4d    %4d\n", subsegnumber,
  ...
  if (regions > 0) {
    ...
    for (regionnumber = 0; regionnumber < regions; regionnumber++) {
      ...
      fGNprintf(outfile, "%4ld   %.17g  %.17g  %.17g  %.17g\n",
...

{
...
#ifdef TRILIBRARY
  if (!b->quiet) {
    GNprintf("Writing Voronoi vertices.\n");
  }
  ...
  *vpointmarkerlist = (int *) NULL;
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    fGNprintf(outfile, "%4ld    %.17g  %.17g", vnodenumber, circumcenter[0],
            circumcenter[1]);
    for (i = 2; i < 2 + m->nextras; i++) {
      /* Interpolate the vertex attributes at the circumcenter. */
      fGNprintf(outfile, "  %.17g", torg[i] + xi * (tdest[i] - torg[i])
                                         + eta * (tapex[i] - torg[i]));
    }
    fGNprintf(outfile, "\n");
#endif /* not TRILIBRARY */
...
#ifdef TRILIBRARY
  if (!b->quiet) {
    GNprintf("Writing Voronoi edges.\n");
  }
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if ((triangleloop.tri < trisym.tri) || (trisym.tri == m->dummytri)) {
        ...
        if (trisym.tri == m->dummytri) {
          ...
          fGNprintf(outfile, "%4ld   %d  %d   %.17g  %.17g\n", vedgenumber,
...

{
  ...
  GNprintf("Mesh quality statistics:\n\n");
  radconst = PI / 18.0;
  degconst = 180.0 / PI;
  for (i = 0; i < 8; i++) {
  ...
  GNprintf("  Smallest area: %16.5g   |  Largest area: %16.5g\n",
         smallestarea, biggestarea);
  GNprintf("  Shortest edge: %16.5g   |  Longest edge: %16.5g\n",
         shortest, longest);
  GNprintf("  Shortest altitude: %12.5g   |  Largest aspect ratio: %8.5g\n\n",
         minaltitude, worstaspect);

  GNprintf("  Triangle aspect ratio histogram:\n");
  GNprintf("  1.1547 - %-6.6g    :  %8d    | %6.6g - %-6.6g     :  %8d\n",
         ratiotable[0], aspecttable[0], ratiotable[7], ratiotable[8],
         aspecttable[8]);
  for (i = 1; i < 7; i++) {
    GNprintf("  %6.6g - %-6.6g    :  %8d    | %6.6g - %-6.6g     :  %8d\n",
           ratiotable[i - 1], ratiotable[i], aspecttable[i],
           ratiotable[i + 7], ratiotable[i + 8], aspecttable[i + 8]);
  }
  GNprintf("  %6.6g - %-6.6g    :  %8d    | %6.6g -            :  %8d\n",
         ratiotable[6], ratiotable[7], aspecttable[7], ratiotable[14],
         aspecttable[15]);
  GNprintf("  (Aspect ratio is longest edge divided by shortest altitude)\n\n");

  GNprintf("  Smallest angle: %15.5g   |  Largest angle: %15.5g\n\n",
         smallestangle, biggestangle);

  GNprintf("  Angle histogram:\n");
  for (i = 0; i < 9; i++) {
    GNprintf("    %3d - %3d degrees:  %8d    |    %3d - %3d degrees:  %8d\n",
...
{
  GNprintf("\nStatistics:\n\n");
  GNprintf("  Input vertices: %d\n", m->invertices);
  if (b->refine) {
    GNprintf("  Input triangles: %d\n", m->inelements);
  }
  if (b->poly) {
    GNprintf("  Input segments: %d\n", m->insegments);
    if (!b->refine) {
      GNprintf("  Input holes: %d\n", m->holes);
    }
  }

  GNprintf("\n  Mesh vertices: %ld\n", m->vertices.items - m->undeads);
  GNprintf("  Mesh triangles: %ld\n", m->triangles.items);
  GNprintf("  Mesh edges: %ld\n", m->edges);
  GNprintf("  Mesh exterior boundary edges: %ld\n", m->hullsize);
  if (b->poly || b->refine) {
    GNprintf("  Mesh interior boundary edges: %ld\n",
           m->subsegs.items - m->hullsize);
    GNprintf("  Mesh subsegments (constrained edges): %ld\n",
           m->subsegs.items);
  }
  GNprintf("\n");
  ...
  if (b->verbose) {
    ...
    GNprintf("Memory allocation statistics:\n\n");
    GNprintf("  Maximum number of vertices: %ld\n", m->vertices.maxitems);
    GNprintf("  Maximum number of triangles: %ld\n", m->triangles.maxitems);
    if (m->subsegs.maxitems > 0) {
      GNprintf("  Maximum number of subsegments: %ld\n", m->subsegs.maxitems);
    }
    if (m->viri.maxitems > 0) {
      GNprintf("  Maximum number of viri: %ld\n", m->viri.maxitems);
    }
    if (m->badsubsegs.maxitems > 0) {
      GNprintf("  Maximum number of encroached subsegments: %ld\n",
             m->badsubsegs.maxitems);
    }
    if (m->badtriangles.maxitems > 0) {
      GNprintf("  Maximum number of bad triangles: %ld\n",
             m->badtriangles.maxitems);
    }
    if (m->flipstackers.maxitems > 0) {
      GNprintf("  Maximum number of stacked triangle flips: %ld\n",
             m->flipstackers.maxitems);
    }
    if (m->splaynodes.maxitems > 0) {
      GNprintf("  Maximum number of splay tree nodes: %ld\n",
             m->splaynodes.maxitems);
    }
    GNprintf("  Approximate heap memory use (bytes): %ld\n\n",
    ...
    GNprintf("Algorithmic statistics:\n\n");
    if (!b->weighted) {
      GNprintf("  Number of incircle tests: %ld\n", m->incirclecount);
    } else {
      GNprintf("  Number of 3D orientation tests: %ld\n", m->orient3dcount);
    }
    GNprintf("  Number of 2D orientation tests: %ld\n", m->counterclockcount);
    if (m->hyperbolacount > 0) {
      GNprintf("  Number of right-of-hyperbola tests: %ld\n",
             m->hyperbolacount);
    }
    if (m->circletopcount > 0) {
      GNprintf("  Number of circle top computations: %ld\n",
             m->circletopcount);
    }
    if (m->circumcentercount > 0) {
      GNprintf("  Number of triangle circumcenter computations: %ld\n",
             m->circumcentercount);
    }
    GNprintf("\n");
  }
...

{
  ...
  if (!b.quiet) {
    gettimeofday(&tv2, &tz);
    if (b.refine) {
      GNprintf("Mesh reconstruction");
    } else {
      GNprintf("Delaunay");
    }
    GNprintf(" milliseconds:  %ld\n", 1000l * (tv2.tv_sec - tv1.tv_sec) +
  ...
  if (!b.quiet) {
    ...
    if (b.usesegments && !b.refine) {
      GNprintf("Segment milliseconds:  %ld\n",
...
#ifndef CDT_ONLY
    if (b.quality) {
      GNprintf("Quality milliseconds:  %ld\n",
  ...
  if (b.nonodewritten || (b.noiterationnum && m.readnodefile)) {
    if (!b.quiet) {
#ifdef TRILIBRARY
      GNprintf("NOT writing vertices.\n");
#else /* not TRILIBRARY */
      GNprintf("NOT writing a .node file.\n");
#endif /* not TRILIBRARY */
    }
  ...
  if (b.noelewritten) {
    if (!b.quiet) {
#ifdef TRILIBRARY
      GNprintf("NOT writing triangles.\n");
#else /* not TRILIBRARY */
      GNprintf("NOT writing an .ele file.\n");
#endif /* not TRILIBRARY */
    }
  } else {
#ifdef TRILIBRARY
  ...
  if (b.poly || b.convex) {
    ...
    if (b.nopolywritten || b.noiterationnum) {
      if (!b.quiet) {
#ifdef TRILIBRARY
        GNprintf("NOT writing segments.\n");
#else /* not TRILIBRARY */
        GNprintf("NOT writing a .poly file.\n");
#endif /* not TRILIBRARY */
      }
    } else {
#ifdef TRILIBRARY
  ...
  if (!b.quiet) {
    ...
    GNprintf("Total running milliseconds:  %ld\n",
...
TOOLS_EXPORTS void ReleaseStruct(struct triangulateio *st)
{
	...
	if (st->pointattributelist != (REAL *) NULL)
		free(st->pointattributelist);
	if (st->pointmarkerlist != (int *) NULL)
		free(st->pointmarkerlist);

	if (st->trianglelist != (int *) NULL)
		free(st->trianglelist);
	if (st->triangleattributelist != (REAL *) NULL)
		free(st->triangleattributelist);
	if (st->trianglearealist != (REAL *) NULL)
		free(st->trianglearealist);
	if (st->neighborlist != (int *) NULL)
		free(st->neighborlist);

	if (st->segmentlist != (int *) NULL)
		free(st->segmentlist);
	if (st->segmentmarkerlist != (int *) NULL)
		free(st->segmentmarkerlist);

	if (st->holelist != (REAL *) NULL)
		free(st->holelist);

	/*if (st->regionlist != (REAL *) NULL)
		free(st->regionlist);*/

	if (st->edgelist != (int *) NULL)
		free(st->edgelist);
	if (st->edgemarkerlist != (int *) NULL)
		free(st->edgemarkerlist);
	if (st->normlist != (REAL *) NULL)
		free(st->normlist);
}
...
TOOLS_EXPORTS int InitStruct(struct triangulateio *st, int npt, int natt, int nseg)
{
	...
	if (npt) {
		st->pointlist = (REAL *) malloc(npt * 2 * sizeof(REAL));
		if (st->pointlist == NULL)
			return 0;
		st->pointattributelist = (REAL *) malloc(npt * natt * sizeof(REAL));
		if (st->pointattributelist == NULL)
			return 0;
	} else {
	...
	st->numberofsegments = nseg;
	if (nseg) {
		st->segmentlist = (int *) malloc(nseg * 2 * sizeof(int));
		if (st->segmentlist == NULL)
			return 0;
	} else {
...
/*----------------------------------------------------------------------------
!@  fine
!@	ripristino dopo uscita anomala dal modulo di triangolazione
!
!----------------------------------------------------------------------------*/
void Terminate(short k)
{
