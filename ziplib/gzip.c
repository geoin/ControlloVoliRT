...
local int treat_file   OF((char *iname, char *oname));
...
int (*work) OF((int infile, int outfile)) = NULL; /* function to call */
...
# endif
	    time_stamp = (long) istat.st_mtime;
...
int _lzw(const char* nome, char* onome) {
	...
	strcpy(innome, nome);
	return treat_file(innome, onome);
}
local int treat_file(char* iname, char* oname)
{
    ...
    time_stamp = no_time && !list ? 0 : (long) istat.st_mtime;
    ...
    if ( decompress ) {
		...
		if ( method < 0 ) {
		 close(ifd);
		 return 0;               /* error message already emitted */
		}
    }
    /*if (list) {
        do_list(ifd, method);
